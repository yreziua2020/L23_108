...
void getWeatherData0() {
  if (f_govorit_fraz) { if (printCom) { Serial.println("потом обновим погоду, говорим фразу");   } return ; }
  ...
  // WiFi.hostByName(weatherHost0.c_str(), pogodaIP, 2000); //если передать айпи а не адрес то все окей, даже если айпи не пингуеться
   if(WiFi.hostByName(weatherHost0.c_str(), pogodaIP)) //если инет есть то бысто даже если непверный адрес// при отсутвиии интернет подвисает по  таймауту 
  ...
  { 
     if (printCom) Serial.println("  Not connection server!!!");  if (updateForecast++ >= 1440) weatherString = tWeatrNot;   return;  
  }
