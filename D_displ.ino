...
void printStringWithShift(const char* s, int shiftDelay) {                                                    // коли працює ця функція, основний цикл зупиняється
  ...
  {  printCharWithShift(*s, shiftDelay);  s++;    
     server.handleClient();  
       ...
       if (one_f1==1||one_f2==1||one_f3==1||one_f4==1||one_f5==1||one_f6==1||one_f7==1) {play_frazi(2,7,100, hour , minute+100);} 
      #endif
        //-------------------------PAJ7620-------------------------------    
...
unsigned char convert_UA_RU_PL_DE(unsigned char _c) {
  ...
  return c; //добавил чтоб убрать ошибку, начиная с версии SDK 3*** .Если ноль то невсе буквы возвращает "c" вроде нормаально 
 }
/*
//==========
