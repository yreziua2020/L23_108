...
uint16_t rawData_info[71] = {9056, 4484,  622, 1628,  592, 532,  566, 552,  568, 554,  590, 532,  590, 532,  592, 530,  592, 528,  590, 532,  590, 1654,  568, 1682,  590, 1656,  564, 1682,  592, 1660,  590, 1656,  592, 1658,  564, 554,  594, 528,  590, 1660,  592, 526,  596, 526,  564, 558,  592, 1656,  596, 526,  564, 1686,  590, 1656,  592, 526,  568, 1682,  564, 1682,  594, 1656,  564, 554,  564, 1682,  592, 40026,  9062, 2218,  564};  // NEC 807F22DD
...
//--------------------------IRsend-----------------------------------------------------
    ...
    //#include <Tone32.h>     //т
    #include <ESPmDNS.h>
  //  WebServer server(80); //было предуприждение поэтому закоментирова
...
//uint16_t trek=222;   //номер мелодии для будильника  //220
//----------------------------------------------------------
...
byte timeDay = 5;
byte timeNight = 24;
bool  f_dny = 1; //флаг дня для пульта только вроде я добавил
int rotate = 90;//90
...
//-----------------------------------------------------------------------------------------------------------------------------------------
float timeZone = 4.0;     //было 2          //также меняем тайм зону в функции обновления времени по вебу handle_timepc()                                                            //  часовий пояс  
int  timeZone_p = 3; //для погоды было 3 летом /на зиму 2
float hourCorr;
...
//*********************************************************************************************************************************************************************************************************************************************************************
void loop() {  
 static int32_t  pl_timer=millis();
...
anglihe();
 ...
 {                                                           
    ...
    if (volBrightnessAuto) { /*levelBridhtness=map(analogRead(brightPin),1023,0,2,15);*/ 
 ...
 if (secFr == 0 && second == 25 && !alarm_stat) {sensorsAll();  /*dav_opros();kol_dav++;*/}//читаем значеение датчков  if (printCom){Serial.println("Обновление датчиков");};
  ...
  if (secFr==0)  {   
      ...
      {     //суда влетает постоянно когда сработал будильник
        ...
        if (!alarm_stat && alarm_numer!=255 && !alarm_hold)  {alarm_stat=1; }
                               ...
                               bip_bud_end(); //будильник всавка часов если играет или посто говорит время после окончания минуты
  ...
  if (secFr == 0) {  //КАЖДУЮ  новую секуду влетаем суда только раз 
    //if (minute == 0) { // minute % 5 == 1   //1, 6, 11, 16...56 хв.
     if (minute==1 || minute==22  ) {  //когда показывать напоминания
          #ifdef d_104
 ...
 // if (minute % 5 == 1) {if ( pred_dav!=pressBmp) {if (pressBmp>pred_dav){nask_dav=int(pressBmp-pred_dav); dav_pov=1;} else {dav_pov=0; nask_dav=int(pred_dav-pressBmp);} pred_dav=pressBmp;}}
  ...
  {  if (hour==3 && minute==1 && second==10 )  {   if (rtcStat) {getRTCDateTime(); hour=hour_rtc; minute=minute_rtc; second=second_rtc;day=day_rtc; month=month_rtc; year=year_rtc; dayOfWeek=dayOfWeek_rtc; if (printCom) {Serial.println("RTC update: "+String(hour)+":"+String(minute)+":"+String(second)+"   "+String(day)+"."+String(month)+"."+String(year)+" D="+String(dayOfWeek));}  } }
    ...
    {                                                                                                                                                                                           //елси время вывода разрешает отображать то выводим прогноз 
      ...
      { if (updateForecast > 0 && updateForecast < 360) { if(!f_angl)  {printStringWithShift(("  (" + tWeatrTN + String(updateForecast) + tMin + ")        ").c_str(), timeScrollSpeed);} perk_sehas_segof_zavt=1;}//нет обновления погоды сколькото минут, выводит соообщение
