//#define _ver "v1"   //если  вместо хай можно напмсать версию
...
void loop() {  
 ...
 //paj7620_t.obrabotka_paj7620();
  //if (f_iz_znach!=f_iz_znach) {f_iz_znach=znach_t; Serial.println(znach_t);}
    ...
    if (pr_bip_full) {b_time_full_ad();} // если установлен флаг говориться фраза то  постоянно  влетает а там проговариваем фразы из масива за ранее созданного
 ...
 if (minute==0 && second==0 && secFr==0 && (hour>=kuOn && hour<kuOff)) { f_kuku=1; play_frazi(6 ,212, dayOfWeek+30, day+40, month+80, hour );}  //при куку не говорим минуты минуты только с веба
